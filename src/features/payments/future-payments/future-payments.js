...
export const SetupFuturePaymentScreen = () => {
  const [email, setEmail] = useState('');
  const [paymentError, setPaymentError] = useState();
  const [offSessionLoading, setOffSessionLoading] = useState(false);
  const [retrievedPaymentIntent, setRetrievedPaymentIntent] = useState(null);
  ...
  const createSetupIntentOnBackend = async customerEmail => {
  ...
  const handleRetrievePaymentIntent = async clientSecret => {
